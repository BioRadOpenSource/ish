...
struct Profile[SIMD_U8_WIDTH: Int, SIMD_U16_WIDTH: Int]:
    # TODO SG: add signed profile types
    ...
    fn generate_query_profile[
        ...
        query: Span[UInt8], read score_matrix: ScoringMatrix, bias: UInt8 = 0
    ) -> List[SIMD[T, size]]:
...
fn ssg_align[
    ...
    *,
    ...
    if profile.profile_byte:
        # print("Doing Byte SW")
        bests = sg[DType.int8, SIMD_U8_WIDTH](
            reference,
        ...
        if profile.profile_word and bests.best.score == 255:
            bests = sg[DType.int16, SIMD_U16_WIDTH](
                reference,
                ReferenceDirection.Forward,
                profile.query_len,
                gap_open_penalty.cast[DType.int16](),
                gap_extension_penalty.cast[DType.int16](),
                profile.profile_word.value(),
                profile.word_vectors,
                -1,
                profile.bias.cast[DType.int16](),
            )
            bests.best.score -= profile.bias.cast[DType.int16]()
            used_word = True
...
fn sg[
    ...
    """
    # TODO: the profile needs to be created with bias set to 0
    # TODO: are UWidth == IWidth?

    # TODO: depends on the SG type
    p_vecs.zero_out()
    p_vecs.init_columns(len(reference))

    # TODO: max score will come from elsewhere, based on SG type?
